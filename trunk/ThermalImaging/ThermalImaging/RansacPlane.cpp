...
void RansacPlane::setInlierDistance(float dis) {
	// sets the distance from the plane when a point will still be considered an inlier
	inlierDistance = dis;
}

void RansacPlane::setPoints(float** ps, int no) {
...
void RansacPlane::setIterations(int is) {
	// sets the number of iterations Ransac should take to find the best plane
	iterations = is;
}

void RansacPlane::findBestPlane(int planeNumber, int &numberOfPointsOnBestPlane, Vector3f &origin, Vector3f &normal, int* bestPoints, int* pointList) {
...
int* RansacPlane::convertToNumberList(int* pointsUsed, int numberOfPoints, int numberOfPointsOnPlane, int planeNumber) {
/*	if (planeNumber == 2)
	{
		int* blee;
		blee = new int[1];
...
void RansacPlane::printToFile(QString fileName, int* data, int nElements) {
